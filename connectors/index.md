...
Turn Jira issues and (opt-in) comments into **project memory** under `project:<KEY>`. API-token auth, project allowlist, no-email user fields (displayName / username / accountId), ADF / plain-text → text, redaction. Pull mode **plus** a webhook receiver (`statewave-connectors listen jira`) that verifies Jira admin webhooks' `X-Hub-Signature` HMAC-SHA256 in real time. Opt-in **status transitions** (`--include transitions` → `jira.issue.transition`, also emitted by the webhook) and opt-in **sprint context** (`--sprint-field`). **Jira Cloud** (REST v3) is exercised end-to-end; **Jira Server / Data Center** (`--deployment server`, REST v2, PAT/Basic auth) is implemented and unit-tested, but **live validation against an on-prem instance is still pending**. `jira.issue.created`, `jira.issue.resolved`, `jira.comment.created`, `jira.issue.transition`. → [`@statewavedev/connectors-jira` README](https://github.com/smaramwbc/statewave-connectors/blob/main/packages/jira/README.md)

### Database (preview)

Ingest **selected external rows** from a relational database into Statewave memory. One package, four dialects — **PostgreSQL, MySQL, MariaDB, MSSQL**. Read-only, allowlisted table or operator `SELECT`, selected columns, required `--max-rows`, `${ENV}` secrets, no schema-wide data dump, no mutation queries, no binary ingestion. An **opt-in schema-metadata mode** (`--mode schema --tables <allowlist>`) ingests catalog metadata — columns, primary key, indexes — for an explicit table allowlist and **never reads data rows** or crawls the instance (`database.schema`). This is a **source connector**, not a Statewave storage backend — Statewave's own storage remains PostgreSQL + pgvector. `database.row`, `database.schema`. All four dialects — PostgreSQL, MySQL, MariaDB, and MSSQL — are live-verified (MSSQL has no per-session read-only flag, so use a least-privilege read-only login). → [`@statewavedev/connectors-database` README](https://github.com/smaramwbc/statewave-connectors/blob/main/packages/database/README.md)
