...
- [x] **Cross-machine query embedding cache** — Postgres-backed `query_embedding_cache` table (alembic `0014_query_embedding_cache`) shared across all Fly machines. Wraps the in-process LRU as L2: L1 hit → L1 → API. A 30s cross-machine OpenAI latency spike on the first hit per machine is eliminated; warm calls are sub-second regardless of which machine handles them. 24h TTL, composite (text, model) key so model rotations don't alias, opportunistic cleanup on write.
