...
**v1.5.0** — timeline and resolutions pagination with `newest_first` recent-end windows and per-collection has-more flags; runtime dependencies (numpy, httpx) declared so the published image serves every admin endpoint. See [`architecture/overview.md`](architecture/overview.md#version-history).
